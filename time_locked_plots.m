function time_locked_plots(figure_dir,preprocessed_data_file,time_lock_firing,epoch_data,infodata,taskdata,unit_names,smval)
...
if ~isfield(infodata,'shuffled_90_percentile')
    for event = 1:25
        for unit = 1:num_units
            for index= 1:7
                infodata.shuffled_90_percentile(event,unit,index)= NaN;
            end
        end
    end
end
...
switch task_type
    case 'ListSQ_Sequence'
        ...
        for unit = 1:num_units
            ...
            for event = 2:13
                time_seq_1 = time_lock_firing{event,unit}(which_sequence == 1,:);
                time_seq_2 = time_lock_firing{event,unit}(which_sequence == 2,:);
            ...
            hold on
            bar(means')
            errorbar(means',stds'./sqrt(numpoints'));
            ylabel('Firing Rate')
            xlabel('Epoch')
            ...
            save_and_close_fig(figure_dir,[preprocessed_data_file(1:end-11) '_' unit_names.name{unit} '_Sequence-time_locked_analysis']);
        end
    ...
    case 'ListSQ_List'
        ...
        for unit = 1:num_units
            ...
            novel_time_lock_firing = time_lock_firing{16,unit}(novel_vs_repeat == 1,:);
            repeat_time_lock_firing = time_lock_firing{16,unit}(novel_vs_repeat == 2,:);
            ...
            novel_time_lock_firing = time_lock_firing{17,unit}(novel_vs_repeat == 1,:);
            repeat_time_lock_firing = time_lock_firing{17,unit}(novel_vs_repeat == 2,:);
            ...
            save_and_close_fig(figure_dir,[preprocessed_data_file(1:end-11) '_' unit_names.name{unit} '_List-time_locked_analysis']);
        end
    ...
    case 'cvtnew'
        ...
        for unit = 1:num_units
            ...
            [s(event),ylims(event)] = make_subplot4([3,3],event,time_lock_firing{event,unit},...
                epoch_data.event_t0(event),250,epoch_data.event_names{event},....
                infodata.rate(event,unit),infodata.shuffled_95_percentile(event,unit),...
                infodata.shuffled_90_percentile(event,unit),smval,epoch_data.trial_duration,twin);
            ...
            save_and_close_fig(figure_dir,[preprocessed_data_file(1:end-11) '_' unit_names.name{unit} '_cvtnew-time_locked_analysis']);
        end
    case 'Sequence'
        which_sequence = taskdata.which_sequence;
        for unit = 1:num_units
            ...
            means(14) = nanmean(temp_data);
            stds(14) = nanstd(temp_data);
            numpoints(14) = sum(~isnan(temp_data));
...
if inforate > info95
    title(['Bits_{95} = ' num2str(inforate)]);
elseif inforate > info90
    title(['Bits_{90} = ' num2str(inforate)]);
end
ylimit = max(y)*1.25;
end
...
if long(2) > size(time_matrix,2)-twin
    long(2) = size(time_matrix,2)-twin;
end
