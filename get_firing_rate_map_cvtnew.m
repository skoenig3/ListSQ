function [ratemaps,timemaps,split_eyepos,split_spike_times] = get_firing_rate_map_cvtnew(trial_data,imageX,imageY,binsize,H,Fs,type)
...
%and spike locations.
%
% same as get_firing_rate_map.m except doesn't remove pixels with zero
% coverage within the dot space 
%
% Inputs:
%   1) trial_data:
...
elseif strcmpi(type,'even_odd')
    ...
    function [filtered_time] = filter_time(eyepos,imageX,imageY,Fs,binsize,H)
        ...
        %         filtered_time(zero_bins) = NaN;%modified to not remove pixels
        %         with zero coverage
