function ImportSequenceRecordingData(data_dir,cch25_file,sequence_file,item_set,multiunit)
...
%   3) sequence_file: plexon_nex file containing data from the ListSQ task
...
cfg=[];
cfg.channel = {};
cfg.dataset       = [data_dir sequence_file];
cfg.trialfun      = 'trialfunSequence';
...
[tform] = Calibrate_Plexon_EyeData(data_dir,cch25_file,'affine'); %get the calibration function

% replaces outliers (more than 1 dva outside image with NaNs not sure how Cluster Fix will process that
for t = 1:length(data(end-1).values);
    x = data(end-1).values{t}; %second to last index in structure array is horizontal eye data;
    y = data(end).values{t}; %last index in structure array is vertical eye data;
...
for t = 1:length(data(end-1).values);
    if ~isempty(data(end-1).values{t})
        fixationstats{t} = ClusterFix_Plexon([data(end-1).values{t};data(end).values{t}]);
    end
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%
%%%---Save the Data---%%%
save([data_dir sequence_file(1:end-11) '-preprocessed.mat'],'cch25_file','cfg','data','multiunit',...
    'fixationstats','hdr','item_set','sequence_file','num_units','waveforms','tform','item_set');
disp(['Data  from ' sequence_file ' Successfully Preprocessed and Saved'])
end