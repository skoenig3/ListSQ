% Population Saccadic Modulation modified from
% Population_Fixation_Modulation Seth Konig 4/5/17
%
% NOT RECHECKED FOR BUGS
%
...
flipped_avg_saccade_firing_rates = []; %significant firing rates +/- max
all_firing_rates = []; %significant + non-significant
avg_saccade_firing_rates_limited =[];%for significant ones limted to duration of 1 preceding saccade+ 1 fixation
...
for monk = 2:-1:1
    ...
    for sess = 1:length(session_data)
        ...
        for unit = 1:num_units
            if isempty(temporal_info.saccade.shuffled_rate{unit})
                continue
            elseif ~isempty(temporal_info.saccade.shuffled_rate{unit}) && (length(temporal_info.saccade.shuffled_rate{unit}) < 1000)
            ...
            if (temporal_info.saccade.shuffled_temporalstability_prctile(1,unit) > 95) ... %significant stability
                ...
                %---Fixation Algined Firing Rate Curves---%
                info = saccade_information{unit}((saccade_information{unit}(:,4) > image_on_twin),:); %fixaion info after image onset
                saccade_firing = saccade_locked_firing{unit}(saccade_information{unit}(:,4) > image_on_twin,:); %fixation aligned spikes after image onset
                firing_rate = nandens(saccade_firing,30,'gauss',Fs,'nanflt');%fixation aligned firing rate
                ...
                flipped_avg_saccade_firing_rates = [flipped_avg_saccade_firing_rates; fr2];
                
                firing_rate = firing_rate/nanmax(abs(firing_rate));
                avg_saccade_firing_rates = [avg_saccade_firing_rates; firing_rate]; %significant firing rates
...
plot([t(1) t(end)],[0 0],'k')
plot([0 0],[-1 1],'k--')
plot(t,mean(avg_saccade_firing_rates),'b')
plot(t,mean(flipped_avg_saccade_firing_rates),'r')
hold off
xlabel('Time from fixation Start (ms)')
ylabel('Normalized Firing Rate')
...
all = [all_firing_rates; avg_saccade_firing_rates];
figure
[m,i] = max(all,[],2);
...
%%
figure
[m,i] = max(avg_saccade_firing_rates(:,200:end),[],2);
[mm,ii] = sort(i);
imagesc(t,[1:size(avg_saccade_firing_rates,1)],avg_saccade_firing_rates(ii,:))
colormap('jet')
hold on
...
fire_rates_non_spatial = avg_saccade_firing_rates(eye_cell_place_cell_status == 0 & ...
    eye_cell_direction_cell_status == 0 & eye_cell_amplitude_cell_status == 0,:);


flipped_rates_non_spatial = flipped_avg_saccade_firing_rates(eye_cell_place_cell_status == 0 & ...
    eye_cell_direction_cell_status == 0 & eye_cell_amplitude_cell_status == 0,:);
...
xlabel('Time from fixation Start')
ylabel('Neuron #')
subtitle('22 Non-spatial Neurons')
axis square